...
// ── TOOLS ────────────────────────────────────────────────────────────────────
server.tool("list_prospects", "Lister les prospects du pipeline avec filtres optionnels", {
...
server.tool("get_prospect", "Obtenir les détails d'un prospect par son ID", { id: z.string().describe("ID du prospect") }, async ({ id }) => {
    const data = await api(`/prospects/${id}`);
    return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
});
server.tool("create_prospect", "Ajouter un nouveau prospect au pipeline (coûte 1 crédit)", {
...
server.tool("update_prospect", "Mettre à jour un prospect existant (coûte 1 crédit)", {
    ...
    const payload = { ...rest, metadata: rest.metadata ? JSON.parse(rest.metadata) : undefined };
    const data = await api(`/prospects/${id}`, { method: "PATCH", body: JSON.stringify(payload) });
    return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
});
server.tool("delete_prospect", "Supprimer un prospect du pipeline", { id: z.string().describe("ID du prospect") }, async ({ id }) => {
    const data = await api(`/prospects/${id}`, { method: "DELETE" });
    return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
});
server.tool("get_stats", "Obtenir les statistiques du dashboard (prospects, campagnes, crédits, limites)", async () => {
    const data = await api("/stats");
    return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
});
server.tool("list_campaigns", "Lister toutes les campagnes WhatsApp", async () => {
    const data = await api("/campaigns");
    return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
});
server.tool("create_campaign", "Créer une nouvelle campagne WhatsApp (coûte 1 crédit)", {
...
server.tool("list_tags", "Lister tous les tags disponibles", async () => {
    const data = await api("/tags");
    return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
});
server.tool("scrape_goafrica", "Scraper GoAfricaOnline pour trouver des prospects (coûte 5 crédits)", {
